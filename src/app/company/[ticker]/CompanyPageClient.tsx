...
type PageData = { ticker: string; generalInfo: any; valuationRatios: any; balanceSheet: { assets: any[], liabilities: any[] }; incomeStatement: any; cashFlow: any[]; ownership: any; management: any[]; };
            ...
            <main className="flex-1 p-4 sm:p-6 lg:p-12 overflow-y-auto">
                <AnimatedSection id="genel-bakis" setActive={setActiveSection}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                        ...
                        <InfoCard title="Son Fiyat" value={`${generalInfo.lastPrice?.toFixed(2) ?? '–'} ₺`} />
                        <InfoCard title="Ana Sektör" value={generalInfo.sector} />
                        <InfoCard title="Alt Sektör" value={generalInfo.subSector} />
                        <InfoCard title="Adres" value={generalInfo.address} isSmall={true} />
                        <InfoCard title="Web Sitesi" value={generalInfo.website} isLink={true} />
                    </div>
                </AnimatedSection>
                <AnimatedSection id="degerleme" setActive={setActiveSection}>
                    <SectionHeader title="Finansal Değerleme" subtitle="Şirket çarpanları ve rasyolar" />
                     <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-6">
                        ...
                        <RatioCircle title="FD/Satış" value={valuationRatios.ps} />
                ...
                <AnimatedSection id="yonetim" setActive={setActiveSection}>
                    ...
                    <div className="space-y-2 mt-6">
                        {management.map((member: any, i: number) => (
                             <details key={i} className="bg-gray-800/50 p-4 rounded-lg cursor-pointer">
                                <summary className="font-semibold text-white">{member.name}</summary>
                                <p className="text-gray-400 mt-2">{member.position}</p>
                            </details>
                        ))}
                    </div>
                </AnimatedSection>
...
// --- TÜM YARDIMCI VE GRAFİK BİLEŞENLERİ (TAM VE OKUNABİLİR HALDE) ---
...
function CustomTreemapContent(props: any) {
    ...
    return (
        <g>
            <motion.rect x={x} y={y} width={width} height={height} style={{ fill: color || (depth === 1 ? '#1f2937' : '#06b6d4'), stroke: '#111827', strokeWidth: 2 }} whileHover={{ fill: '#0e7490' }} transition={{ duration: 0.2 }} />
            {width > 80 && height > 30 && (
...
function IncomeSankeyChart({ data }: { data: any; }) {
    const sankeyData = { revenue: { value: data.revenue, label: "Hasılat" }, cost: { value: data.cost, label: "Satışların Maliyeti" }, grossProfit: { value: data.grossProfit, label: "Brüt Kâr" }, expenses: { value: data.expenses, label: "Faaliyet Giderleri" }, earnings: { value: data.earnings, label: "Esas Faaliyet Kârı" } };
    return (
            ...
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={processedData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                    ...
                    <Bar dataKey="value" stackId="a">
                        {processedData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.isResult ? "#8b5cf6" : entry.value > 0 ? "#10b981" : "#ef4444"} />))}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
        </div>
