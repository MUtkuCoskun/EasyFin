// scripts/isyatirim-all-xlsx.ts
// Kullanım:
//   npx tsx scripts/isyatirim-all-xlsx.ts SASA AUTO TRY 2008 3
//   (ticker, grup[CONSOL|XI_29|AUTO], kur, başlangıç yıl/period (3/6/9/12))
...
const OUT_DIR = path.join('/Users/utku/Downloads', 'isyatirim', TICKER)
...
async function writeXlsx(output: Output) {
  await ensureDir(OUT_DIR)
  const xlsxFile = path.join(OUT_DIR, 'mali-tablo.xlsx')
  const wb = new ExcelJS.Workbook()

  // Tek sayfa: MaliTablo
  const ws = wb.addWorksheet('MaliTablo')

  const periodKeys = output.meta.periodKeys
  const headers = ['itemCode','itemDescTr','itemDescEng', ...periodKeys]
  ws.columns = headers.map(h => ({ header: h, key: h, width: Math.max(12, h.length + 2) }))

  // sayı formatı ve görünüm
  for (let c = 4; c <= headers.length; c++) ws.getColumn(c).numFmt = '#,##0.00;[Red]-#,##0.00'
  ws.views = [{ state: 'frozen', xSplit: 3, ySplit: 1 }]
  ws.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: headers.length } }

  // satırlar
  const keys = Object.keys(output.items).sort((a, b) => {
    const A = output.items[a], B = output.items[b]
    const ka = (A.code || A.tr || A.en || '').toString()
    const kb = (B.code || B.tr || B.en || '').toString()
    return ka.localeCompare(kb, 'tr')
  })
  for (const k of keys) {
    const it = output.items[k]
    const row: Record<string, any> = {
      itemCode: it.code ?? '',
      itemDescTr: it.tr ?? '',
      itemDescEng: it.en ?? ''
    }
    for (const p of periodKeys) row[p] = it.values[p] ?? null
    ws.addRow(row)
  }

  await wb.xlsx.writeFile(xlsxFile)
  console.log(`✔ XLSX yazıldı: ${xlsxFile}`)
}
...
async function run() {
  ...
  if (GROUP === 'AUTO') {
    ...
    // son 4 dönemi iki gruptan da tekrar yaz
